...
class APIRouter {
  ...
  async getListResponse(query, request) {
    ...
    if (query.clone) {
      // mongoose > 6.0
      ret.actualTotal = await query.clone().countDocuments(); /// @todo Use estimatedDocumentCount() if there're no filters?
    } else {
      ret.actualTotal = await query.countDocuments(); /// @todo Use estimatedDocumentCount() if there're no filters?
    }
  ...
  async routeBulkInsert(request, reply) {
    let docs = await this._model.insertMany(request.body);
    let result = [];
    for (let doc of docs) {
      await this.populateIfNeeded(request, doc);
      result.push(await this.docToAPIResponse(doc, request));
    }
    reply.send(result);
  }
